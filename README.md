...
Готов **каркас**: контракты, общие библиотеки, десять запускающихся сервисов,
инфраструктура, сборка и развёртывание. Доменная логика ещё не написана —
у сервисов пока только health-эндпоинты и подключения к транспортам.

Порядок дальнейшей работы — §14 архитектуры. Каждый шаг вводит ровно один новый
архитектурный механизм поверх уже работающих:

1. `auth → hr` — регистрация и событие `auth.user.registered` (проверяет оба транспорта)
2. `hr` целиком — модель найма, графики, расчётный табель
3. `approval` — сага согласования, scope-права
