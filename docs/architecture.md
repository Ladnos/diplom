...
graph LR
    ...
    EX -->|"task.# chat.# video.# approval.#<br/>hr.timesheet.# notification.created"| Q3["gateway.realtime.{instance}"]
...
| `chat.message.sent` | messageId, channelId, authorId, seq, preview, recipients[], hasAttachments | **gateway** — доставка участникам по WS; **notification** — push офлайн-участникам; **analytics** |
| `chat.message.edited` / `chat.message.deleted` | messageId, channelId, seq | **gateway**; **file** (при удалении — refcount вложений) |
| `chat.mention.created` | messageId, channelId, authorId, mentionedIds[] | **notification** — приоритетное уведомление независимо от присутствия |
| `chat.reaction.added` | messageId, channelId, employeeId, emoji | **gateway** |

> **Упоминание — отдельное событие, а не флаг внутри `chat.message.sent`.** У него другой получатель и другая срочность: обычное сообщение не догоняет push'ом того, кто в системе, а упоминание доставляется независимо от присутствия. Флагом это выражалось бы ветвлением внутри правила уведомления, где список упомянутых уже смешан с общим списком получателей.
>
> **Пустой `authorId` в `chat.message.sent` — системная запись** вроде «звонок завершён, 42 мин». Автора у неё нет, и подставлять сюда идентификатор сервиса значило бы завести несуществующего сотрудника, которого потом пришлось бы отфильтровывать в каждом потребителе.
...
| Routing key | Payload | Подписчики и реакция |
|---|---|---|
| `notification.created` | notificationId, userId, title, body, link?, priority | **gateway** — поднять счётчик непрочитанного в открытом окне (§8.1) |

> Единственное событие, которое публикует `notification-service`, и единственное во всей системе, адресованное по `userId`, а не по `employeeId`: лента принадлежит учётной записи, и у части получателей карточки сотрудника нет. Публикуется без outbox — см. §8.1.
...
|---|---|---|---|---|
| `gateway.realtime.{instance}` | api-gateway (каждый инстанс) | `task.#`, `chat.#`, `video.#`, `approval.#`, `hr.timesheet.#`, `notification.created` | — | 100 |
...
| `chat.events` | chat-service | `hr.employee.#`, `hr.hierarchy.changed`, `video.call.ended` | — | 20 |
...
>
> **Один обработчик на всю шину.** Привязка `#` означает, что в `analytics-service` приходит каждое событие системы, а NestJS отвергает сообщение, для которого не нашёл `@EventPattern`. Перечислять полсотни типов поимённо здесь нельзя: каждое новое событие, добавленное в любом сервисе, молча уходило бы в DLQ. Так и вышло — к моменту реализации в `analytics.events.dlq` лежало больше трёхсот сообщений, накопленных заглушкой. Команда `report.generate` разбирается внутри того же обработчика: она тоже подходит под `#`, и при двух подходящих паттернах выбор зависел бы от порядка объявления методов.

> **Привязки `chat.events` пересмотрены при реализации.** Добавлена `hr.hierarchy.changed`: право писать в канал объявлений принадлежит руководителю (ADR-3), а `hr.employee.#` приносит начальника только при создании и правке карточки — перевод внутри оргструктуры идёт отдельным событием, без которого проекция отставала бы молча. Убрана `task.card.assigned`: полезной реакции на неё у чата не нашлось — связи «карточка ↔ канал» в модели нет, а уведомление о назначении и без того отправляет `notification-service`. Привязка без обработчика не бесплатна: NestJS отвечает `nack(requeue: false)` на событие, для которого не нашлось `@EventPattern`, и оно копится в DLQ.
>
> **`assertTopology` только добавляет.** Привязку, удалённую из таблицы, он с существующей очереди не снимает — на стенде она убирается вручную через management API. Свойство стоит помнить: расхождение объявленного и фактического не проявляется ошибкой, а копится в DLQ.
...
> **Особенность `gateway.realtime`.** Очередь объявляется с уникальным для инстанса именем (`gateway.realtime.<instance-id>`, `autoDelete: true`, `durable: false`), потому что WS-обновление должно прийти **на каждый** инстанс gateway, а не одному из них. Остальные очереди — общие: сообщение обрабатывает ровно один консьюмер из пула.
>
> `exclusive: true` здесь использовать **нельзя**, хотя по смыслу очередь именно эксклюзивная: такая очередь привязана к объявившему её подключению и удаляется вместе с ним, а объявление идёт отдельным коротким подключением `assertTopology`. Потребитель подключился бы уже к несуществующей очереди.
>
> DLQ у неё тоже нет — и это осознанно. Realtime-обновление, которое не удалось разложить по соединениям, бессмысленно разбирать вручную: клиент дочитает состояние обычным запросом. Обратная сторона — **отсутствующий обработчик не оставляет следов**. NestJS отвечает `nack(requeue: false)` на сообщение, для которого не нашлось `@EventPattern`, и у обычной очереди такие сообщения копятся в DLQ, а здесь исчезают бесследно. Поэтому обработчики в `realtime.controller.ts` повторяют паттерны привязок из этой таблицы, а не перечисляют события поимённо.
...
Реализация: `apps/api-gateway/src/realtime/` — шлюз соединений (`realtime.gateway.ts`), потребитель очереди (`realtime.controller.ts`), карта разложения событий по комнатам (`fanout.map.ts`), таблица комнат и правил доступа (`rooms.ts`).

**Размножение события между инстансами делает RabbitMQ, а не Socket.IO.** Очередь `gateway.realtime` объявлена `perInstance` (§7.5), поэтому копию получает каждый инстанс и рассылает её своим сокетам. У Socket.IO есть адаптер поверх Redis, решающий ту же задачу, — и подключать его здесь **нельзя**: событие прошло бы оба механизма и пришло клиенту N раз при N инстансах.

**Имя события Socket.IO совпадает с routing key.** Клиент подписывается на `task.card.moved` и получает ровно его, без разбора обёртки `{type, payload}` вручную.

#### Комнаты

| Комната | Кто в ней | Кто решает о доступе |
|---|---|---|
| `user:<userId>` | сам пользователь | токен |
| `employee:<employeeId>` | сам сотрудник | токен |
| `board:<boardId>` | участники доски | **task-service** (`assertMember`) |
| `channel:<channelId>` | участники канала | **chat-service** (ещё нет — вход закрыт) |
| `team:<managerEmployeeId>` | руководитель и те, кто вправе видеть его команду | auth-service, `employee:read` + ownerId |
| `department:<departmentId>` | причастные к табелю отдела | auth-service, `timesheet:read` |
| `presence:<employeeId>` | наблюдающие за активностью сотрудника | auth-service, `employee:read` + ownerId |

**Личных комнат две, а не одна.** Доменные события несут `employeeId` — идентификатор карточки сотрудника, а лента уведомлений и права принадлежат учётной записи с `userId`. Это разные идентификаторы, и у части пользователей (администратор без карточки) `employeeId` нет вовсе. Сокет знает оба из claims токена, поэтому входит сразу в две личные комнаты, и проекция «userId ↔ employeeId» на стороне gateway не нужна.

**Доступ к комнате объекта проверяет сервис-владелец, а не только auth-service.** Права в auth-service отвечают на вопрос «может ли этот человек работать с досками вообще»; без указания владельца объекта `CheckPermission` возвращает «право есть, область не проверялась» и ждёт, что вызывающий сам сузит выборку. Для HTTP это работает — домен фильтрует результат. Для комнаты фильтровать нечего: вход в неё **и есть** доступ. Поэтому `board:` проверяется вызовом `GetBoardMembers`, который внутри делает `assertMember`, — тем же способом, что и HTTP-обработчик доски.

Комната, авторитет которой ещё не реализован, закрыта: подписка на `channel:` отклоняется до появления `chat-service`. Пустить «пока просто так» нельзя — подписка не отзывается до разрыва соединения, и появление сервиса молча превратило бы её в утечку переписки.

#### Жизненный цикл соединения

Токен предъявляется один раз при рукопожатии — в `auth.token`, в строке запроса или в cookie `access_token`. Соединение живёт часами, поэтому момент истечения claims запоминается, и раз в тридцать секунд шлюз закрывает сокеты с истёкшим токеном, отправив перед разрывом `unauthorized {reason: "token_expired"}`. Без этого сокет, открытый с валидным токеном, получал бы события и через сутки после того, как REST для того же пользователя отвечает 401.

Дедупликация входящих событий на gateway не ведётся: своей базы у него нет, а повторно доставленное realtime-событие означает лишь повторную перерисовку уже показанного. Каждое сообщение несёт `eventId` и `occurredAt`, по которым клиент отличает дубль от нового изменения.

#### Что попадает в WebSocket

Наружу уходит **проекция** события, а не доменный payload. `chat.message.sent` несёт полный список получателей, `approval.request.approved` — тело заявки; отправить конверт как есть значило бы раздать всей комнате данные, которые каждому её участнику по отдельности видеть можно, а вместе — не обязательно. Клиент получает идентификаторы и минимум для отрисовки, подробности дочитывает обычным запросом.

Отдельно стоит `notification.created` — единственное событие, которое публикует `notification-service`. Оно поднимает счётчик непрочитанного в открытом окне и адресуется по `userId`. Публикуется **без outbox**: лента уже сохранена в БД и читается запросом, поэтому потеря сигнала стоит одного неподнятого счётчика — ровно то поведение, что было до появления WebSocket.
...
**Как присваивается `seq`.** Инкрементом поля `last_message_seq` в строке канала внутри той же транзакции, что и вставка сообщения. Строка канала при этом блокируется, и второй отправитель дожидается коммита первого — номера получаются разными. Вычислять `max(seq)` по таблице сообщений было бы и медленнее, и неверно: двое, отправившие одновременно, прочитали бы одно и то же значение и заняли бы один номер.

**Повторная отправка.** Клиент присваивает сообщению `clientMessageId` до отправки, и уникальный индекс `(channel_id, client_message_id)` превращает повтор после обрыва связи в конфликт вставки, который сервис разрешает возвратом уже сохранённого сообщения. Без этого нажатие «отправить» на зависшей сети давало бы дубль — а именно на зависшей сети его и нажимают дважды.

**Удаление — пометка, а не удаление строки.** Строка держит номер `seq` и служит корнем ветки для ответов; физическое удаление порвало бы и нумерацию, и ветки, а разрыв в номерах клиент трактует как потерянный пакет и уходит перечитывать историю. Текст при этом стирается: «удалить» должно означать, что содержимого больше нет, а не что его перестали показывать.

Два нижних транспорта живут в `api-gateway` (`realtime/ephemeral.bus.ts`, `realtime/presence.service.ts`) и работают независимо от `chat-service`:

**«Печатает».** Клиент отправляет `typing {channelId}`, шлюз публикует сигнал в канал Redis `typing:<channelId>`, все инстансы получают его по подписке `typing:*` и рассылают в комнату канала. Автору собственный индикатор не приходит: вместе с сигналом передаётся `socketId`, и отправитель исключается из рассылки — именно сокет, а не пользователь, потому что во второй его вкладке индикатор как раз уместен. Не чаще одного сигнала в секунду с соединения.

**Присутствие.** Пока сокет открыт, gateway держит ключ `presence:user:<userId>` со значением-идентификатором инстанса и TTL 90 секунд, продлевая его каждые 30. Упавший инстанс поэтому не оставляет пользователя «вечно онлайн» — продлевать станет некому. Читает ключ `notification-service`, чтобы не слать push тому, кто смотрит на экран (§7.3).

Тонкость на нескольких инстансах: отметку снимает только тот инстанс, который её ставил (сравнение значения и удаление одной Lua-операцией). Иначе закрытие одной вкладки гасило бы присутствие, поставленное другим инстансом. Обратный случай — отметка наша, а на соседнем инстансе соединение осталось — разрешается его же продлением в течение тридцати секунд. Кратковременная ошибка в сторону «офлайн» стоит лишнего push; ошибка в другую сторону означала бы молчание в момент, когда человека пытаются дозваться.

**Прочтение.** `MarkRead` кладёт номер в Redis (`chat:read:<employeeId>`, поле — канал) и добавляет человека в множество «к выгрузке»; счётчик непрочитанного обязан обновиться в тот же момент, иначе прочитанный канал остаётся подсвеченным. Раз в десять секунд `chat-service` забирает пачку через `SPOP` и переносит значения в `read_cursors`.

Курсор движется только вперёд, и это проверяется трижды: скриптом Lua при записи в Redis (две вкладки одного человека), выражением `GREATEST` в `ON CONFLICT` при выгрузке (задержавшаяся выгрузка может прийти после следующей) и взятием максимума из двух источников при чтении. Откат курсора превратил бы прочитанное в непрочитанное — единственная ошибка, которую здесь видно пользователю.

Считаются сообщения, а не разность `last_message_seq − курсор`: собственные и удалённые из подсчёта выпадают, а разность посчитала бы и их. Один запрос на все каналы — раскладка курсоров передаётся массивами и соединяется через `unnest`, иначе на человека с тридцатью каналами пришлось бы тридцать запросов.

При недоступном Redis отметка пишется прямо в базу. Потеря буфера стоит нескольких сообщений, снова ставших непрочитанными, — ошибка в безобидную сторону.
...
`apps/video-service/src/` — `sfu/` (пул воркеров, роутеры, транспорты), `signaling/` (WebSocket), `rooms/` (комнаты, пропуска, gRPC).

**Воркер на ядро.** `mediasoup-worker` — нативный процесс, и медиа идёт через него мимо Node: JavaScript участвует только в управлении, пакеты RTP не проходят через event loop. Один воркер — один поток, поэтому их поднимается по числу ядер, а диапазон UDP-портов делится между ними: они слушают напрямую, и пересечение диапазонов означало бы гонку за порт при старте.

Роутер (комната) целиком живёт внутри одного воркера — перенести его нельзя, поэтому распределение делается один раз, при создании, по наименьшей загрузке. Роутер создаётся при первом ВХОДЕ, а не при создании комнаты: приглашённых может быть десять, а войти — никто.

**`listenIp` и `announcedIp` — разные вещи.** Слушать нужно на всех интерфейсах контейнера (`0.0.0.0`), а объявлять клиенту публичный адрес сервера. Объявишь адрес Docker-сети — ICE навсегда останется в состоянии checking, и это не проявится ошибкой: соединение будет просто молча не устанавливаться.

**Consumer создаётся приостановленным** и включается отдельной командой: иначе пакеты пойдут раньше, чем клиент разберёт ответ и подготовит декодер, и начало чужой речи потеряется.

**Выключение микрофона приостанавливает поток, а не пересоздаёт его.** Пересоздание означало бы новый ICE и новый DTLS на каждое нажатие кнопки — секунды тишины вместо мгновенного отклика.

#### Пропуск вместо access-токена

Сигналинг идёт мимо `api-gateway`, поэтому проверять на нём обычный токен значило бы вызывать `auth-service` при каждом открытии соединения — лишний круг ожидания там, где важна задержка. Вместо этого шлюз, уже проверив права и участие в звонке, просит `IssueJoinToken`, а сигналинг проверяет подпись локально.

Формат намеренно простой: payload в base64url и HMAC-SHA256 от него; полноценный JWT добавил бы только заголовок с алгоритмом — то самое поле, подмена которого исторически и ломала разборщики. Пропуск живёт минуту и не отзывается: он нужен на один вход.

#### Граница реализации

**Запись звонков не реализована** и отвечает `UNIMPLEMENTED`, а не молчаливым согласием. Механизм требует `PlainTransport` в SFU и ffmpeg, принимающего RTP и складывающего файл; ffmpeg в образе нет — ровно как и в `file-service` для команды `media.process`. Ответить «начато», ничего не записывая, хуже отказа: участники были бы уверены, что разговор сохраняется.

**Один инстанс.** Комната привязана к воркеру конкретного процесса, поэтому масштабирование требует sticky-балансировки по `roomId` и переноса состояния комнат в Redis. Для целевых ~500 сотрудников достаточно одного инстанса; ограничение того же рода, что у `file-service` (§9.6), и по той же причине — оно вытекает из принятой границы задачи, а не из недосмотра.

**mediasoup собирается из исходников.** Готовые сборки воркера рассчитаны на glibc и в alpine не работают, поэтому в стадии `builder` образа стоят python, компилятор и ninja. Обратная сторона: `npm ci` в монорепозитории ставит зависимости всех workspace-пакетов, и остальные восемь образов пытались бы собрать тот же воркер без нужных инструментов. У них сборка отключена переменной `MEDIASOUP_WORKER_BIN`.
...
**Загрузка идёт мимо `api-gateway`** — nginx маршрутизирует `/api/files/upload` напрямую в `file-service`. Многомегабайтные тела не должны проходить через сервис, обслуживающий все остальные запросы. Отсюда следствие: `file-service` — единственный сервис, кроме шлюза, который сам проверяет токен. Запрос предъявлен ему, и переложить проверку не на кого.

**Отказ по размеру обязан дочитать тело.** Предел проверяется в процессе чтения, и поток обрывается на первом же лишнем байте — но остаток запроса при этом нужно принять и выбросить. Иначе поток остаётся без потребителя, буфер заполняется, nginx блокируется на записи и отвечает клиенту `504` через пять минут вместо `413` через семьдесят миллисекунд: готовый ответ до него просто не доходит. Объём «дочитывания» ограничен сверху `client_max_body_size`.

Там же вторая тонкость: реагировать на превышение нужно сразу по отказу укладки, а не по событию `close` разборщика multipart. Оборванный файловый поток лишает busboy потребителя, и `close` он уже не эмитит — ожидание этого события даёт тот же `504`.
...
**Кому отдавать — решает не владение файлом.** Вложение к сообщению принадлежит тому, кто его загрузил, а видеть его должны все участники канала — и перестать видеть, когда их из канала исключат. Значит, право на файл выводится из права на сущность, в которой он висит, и знает об этом только сервис-владелец сущности. Порядок проверок в `AccessService` идёт от дешёвых к дорогим:

1. собственный файл — решается локально;
2. `CheckPermission(file, read, ownerId)` — один вызов `auth-service`, покрывает администратора и руководителя;
3. обход привязок: `chat.CanAccessMessage` для сообщения, `task.GetCard` для карточки (он сам проверяет участие в доске), `employee:read` по владельцу для аватара.

Достаточно одной привязки, дающей доступ: файл, разосланный в два канала, виден участнику любого из них. Обход ограничен сверху пятью привязками — файл со ста ссылками не должен превращать одно скачивание в сотню межсервисных вызовов. Отказ сервиса-владельца и его недоступность трактуются одинаково: «не подтверждено».

Зависимость односторонняя. `file-service` вызывает `chat` и `task`, но не наоборот: они лишь публикуют события об удалении своих сущностей. Поэтому и привязку вложения к сообщению делает `api-gateway` после его создания, а не `chat-service` — встречный вызов замкнул бы пару сервисов друг на друга ради операции, которую шлюз и так выполняет, зная и файл, и сущность.
...
Разделение режимов: **приватные** документы и вложения — через `X-Accel-Redirect` с проверкой прав; **полупубличные** аватары и превью — через `secure_link` с TTL 24 часа. Выдать подписанную ссылку на приватный файл нельзя — это обошло бы проверку прав, ради которой он приватным и сделан.

**Секрет подписи — одна переменная на два процесса.** Ссылку выдаёт `file-service`, проверяет её nginx, и склейка обязана совпасть до символа: срок, путь, адрес клиента, пробел, секрет; MD5; base64url без выравнивания. Расхождение не даёт ошибки — оно даёт `403` на все ссылки разом, без единой записи в журнале приложения. Поэтому `nginx.conf` смонтирован как шаблон, а `${FILE_SIGNED_LINK_SECRET}` подставляется штатным механизмом образа при старте; список подставляемых имён задан явно (`NGINX_ENVSUBST_FILTER`), иначе `envsubst` съел бы и собственные переменные nginx вроде `$uri` и `$remote_addr`.

Префикс `/media/`, а не `/files/`: последний занят интерфейсом под раздел «Файлы», и локация подписанных ссылок перехватывала бы его раньше SPA-маршрута, отдавая `403` на обычный переход. Внутренний путь доставки не должен занимать пользовательский адрес — у первого имя произвольно, у второго нет.

**Адрес клиента в подписи** берётся из заголовка `X-Real-IP`, который ставит тот же nginx: до сервиса доходит адрес самого прокси, и подпись по нему не сошлась бы ни у одного клиента. Обратная сторона привязки к адресу — смена сети рвёт уже выданные ссылки; при сроке жизни в сутки это дешевле, чем ссылка на документ, гуляющая по переписке.

### 9.5. Эксплуатация

| Задача | Решение |
|---|---|
| Сборка мусора | Команда `file.gc.run` и собственный таймер раз в 6 часов: загрузки с `refcount = 0` старше 7 суток удаляются, следом — содержимое, на которое не ссылается ни одна загрузка. Отсрочка защищает от гонки «файл загружен, но ещё не привязан»: между прикреплением файла и отправкой сообщения человек успевает написать текст |
| Квоты | Лимит на пользователя; при превышении — событие `file.quota.exceeded` и `413`. Считается **логический** объём, а не место на диске: при подсчёте по физическому занятость человека менялась бы от действий постороннего — коллега удалил свою копию того же файла, и у первого «появилось место» |
...
> **Отклонение от плана при реализации.** WebSocket-слой вынесен из шага «chat» в самостоятельный и сделан раньше него — сразу после `notification`. Причина в том, что слой оказался нужен трём уже готовым подсистемам (Kanban, согласования, счётчик уведомлений), а не одному чату, и откладывать его до чата значило бы держать очередь `gateway.realtime` объявленной, но не читаемой: события в неё шли, обработчиков не было, и все они бесследно отбрасывались (§7.5). Фактический порядок: `auth → hr → approval → task → notification → WebSocket → chat → file → video → analytics`.
