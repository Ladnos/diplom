...
> **Про `notification-service`.** Он держит локальную денормализованную копию контактов (e-mail, push-подписка, настройки уведомлений, тихие часы), наполняемую событиями `hr.employee.created` / `hr.employee.updated`. Это сохраняет свойство «сервис без входящих gRPC» и даёт устойчивость: рассылка работает даже при недоступном `hr-service`. Цена — разовая заливка контактов при первом развёртывании (CLI-команда `backfill:contacts`) и метод `GetContacts` как fallback.
